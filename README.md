...
python build_education.py      # 原住民成年人口學歷結構（縣市層級）
...
上面全部是「在學的原民生」。`build_education.py` 算的是**存量**——已經在社會上的
原住民受過多少教育。兩者合起來才看得出高教管道的效果。

113 年底，全國 15 歲以上原住民 **490,336 人**，專科以上 **35.0%**。縣市差距很大：
...
都會縣市高、原鄉低，跨度超過 20 個百分點。

### ⚠️ 這份資料的三個限制

1. **基準日是民國 113 年 12 月底，不是學年度。** 這是人口統計的存量，教育部那邊是
   學年度的在學人數，**不可以畫在同一條時間軸上**，也不能互相當分子分母。
2. **只能到縣市層級。** 原始空間單元是「發布區」，代碼長成 `A1501-24`：`A` ＋ 縣市序號
   ＋ 2 碼鄉鎮序號。縣市可以還原（`A15` → 10015 花蓮縣，見各檔 `Info.InCountyId`），
   但那 2 碼是 SEGIS 內部序號、**不是 8 碼行政區代碼**，沒有官方對照表就接不回鄉鎮市區。
   要做鄉鎮層級請先解決這件事，不要用中文名稱硬 join。
